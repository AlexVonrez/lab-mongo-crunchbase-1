        ...
        switch (option) {
          ...
          case "3":
           ...
           db.collection("companies").find({"relationships.person.permalink": "david-ebersman" }, { "name": 1, _id: 0 }) .toArray((error, result) => {
            ...
            db.collection('companies').find({ $and : [{tag_list: { $regex: "social-networking" } },{ founded_year: { $gte: 2002 } },{ founded_year: { $lte: 2016 } },],}, { "products.name": 1, _id: 0 }).count((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
            ...
            db.collection('companies').find({ $and:[{tag_list:{ $regex: "social-network" }},{ founded_year: { $gte: 2002 } },{ founded_year: { $lte: 2016 } },{ "offices.city": "New York" },],}, { "products.name": 1, _id: 0 }).count((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
